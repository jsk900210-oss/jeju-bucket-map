  ...
  return (
    <main>
      <header className="topbar">
        ...
        <button className="user-chip" onClick={() => move("profile")}><span>👤</span><b>로그인</b></button>
      </header>
      ...
      {tab === "profile" && <section className="subpage shell profile-page">
        <div className="profile-head"><div className="avatar">👤</div><div><span className="eyebrow">ACCOUNT</span><h1>내 계정 만들기</h1><p>Google 인증 후 서비스 내부 사용자 계정이 생성됩니다.</p><div className="stats"><span><b>0</b> Join</span><span><b>0</b> 신청</span><span><b>0</b> 참여 기록</span></div></div></div>
        <div className="keyword-panel" style={{marginTop: 24}}><div className="panel-title"><div><span className="mini-label">GOOGLE OAUTH</span><h2>Google 계정으로 시작하기</h2></div><span className="test-badge">연결 설정 필요</span></div><p>서비스는 비밀번호를 저장하지 않습니다. Google이 사용자를 인증하면 내부 사용자 ID를 만들고 Join 생성·신청·취소·참여 기록을 계정별로 관리합니다.</p><button className="primary" disabled title="Google OAuth Client ID와 배포 환경 설정이 필요합니다.">Google 로그인 준비 중</button></div>
      </section>}
